/* ==========================================
   PENGAJUAN - Page Specific Scripts
   ========================================== */
...
function openDetail(idx) {
    ...
    statusEl.style.color = d.status === 'Disetujui' ? '#198754' : d.status === 'Ditolak' ? '#dc3545' : '#d97706';
    ...
    // Dosen pembimbing: show dropdown for Menunggu, text for others
...
btnTutupDetail.addEventListener('click', () => detailModal.classList.remove('show'));
btnBatalDetail.addEventListener('click', () => detailModal.classList.remove('show'));
...
// From detail modal button
document.getElementById('btnApproveDetail').addEventListener('click', function() {
    detailModal.classList.remove('show');
    approveModal.classList.add('show');
});

btnBatalApprove.addEventListener('click', () => approveModal.classList.remove('show'));
btnConfirmApprove.addEventListener('click', () => {
    approveModal.classList.remove('show');
    // Update dosen pembimbing jika dipilih
    const dosenSelect = document.getElementById('det-dosen-select');
    if (dosenSelect && dosenSelect.value) {
        dataPengajuan[currentIdx].dosen_pembimbing = dosenSelect.value;
    }
    alert('Pengajuan ' + dataPengajuan[currentIdx].id + ' telah disetujui!\nDosen Pembimbing: ' + dataPengajuan[currentIdx].dosen_pembimbing);
});
...
// From detail modal button
document.getElementById('btnRejectDetail').addEventListener('click', function() {
    detailModal.classList.remove('show');
    rejectModal.classList.add('show');
});

btnBatalReject.addEventListener('click', () => rejectModal.classList.remove('show'));
btnConfirmReject.addEventListener('click', () => {
    rejectModal.classList.remove('show');
    alert('Pengajuan ' + dataPengajuan[currentIdx].id + ' telah ditolak.');
});
